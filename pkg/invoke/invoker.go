...
import (
	...
	"github.com/orion101-ai/orion101/pkg/hash"
	...
	options := gptscript.Options{
		GlobalOptions: gptscript.GlobalOptions{
			Env: append(run.Spec.Env,
				...
				"ORION101_SERVER_URL="+i.serverURL,
				"ORION101_TOKEN="+token,
				"ORION101_RUN_ID="+run.Name,
				"ORION101_THREAD_ID="+thread.Name,
				"ORION101_WORKFLOW_ID="+run.Spec.WorkflowName,
				"ORION101_WORKFLOW_STEP_ID="+run.Spec.WorkflowStepID,
				"ORION101_AGENT_ID="+run.Spec.AgentName,
				"GPTSCRIPT_HTTP_ENV=ORION101_TOKEN,ORION101_RUN_ID,ORION101_THREAD_ID,ORION101_WORKFLOW_ID,ORION101_WORKFLOW_STEP_ID,ORION101_AGENT_ID",
			),
...
func toSubCall(output string) *v1.SubCall {
	var call call
	if err := json.Unmarshal([]byte(strings.TrimSpace(output)), &call); err != nil || call.Type != "Orion101SubFlow" || call.Workflow == "" {
		return nil
	}
