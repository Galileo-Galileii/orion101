...
import (
	...
	"github.com/orion101-ai/nah"
	"github.com/orion101-ai/nah/pkg/leader"
	"github.com/orion101-ai/nah/pkg/router"
	"github.com/orion101-ai/orion101/pkg/aihelper"
	"github.com/orion101-ai/orion101/pkg/api/authn"
	"github.com/orion101-ai/orion101/pkg/api/authz"
	"github.com/orion101-ai/orion101/pkg/api/server"
	"github.com/orion101-ai/orion101/pkg/credstores"
	"github.com/orion101-ai/orion101/pkg/events"
	"github.com/orion101-ai/orion101/pkg/gateway/client"
	"github.com/orion101-ai/orion101/pkg/gateway/db"
	gserver "github.com/orion101-ai/orion101/pkg/gateway/server"
	"github.com/orion101-ai/orion101/pkg/invoke"
	"github.com/orion101-ai/orion101/pkg/jwt"
	"github.com/orion101-ai/orion101/pkg/proxy"
	"github.com/orion101-ai/orion101/pkg/smtp"
	"github.com/orion101-ai/orion101/pkg/storage"
...
type Config struct {
	...
	DevMode                    bool   `usage:"Enable development mode" default:"false" name:"dev-mode" env:"ORION101_DEV_MODE"`
	DevUIPort                  int    `usage:"The port on localhost running the dev instance of the UI" default:"5173"`
	AllowedOrigin              string `usage:"Allowed origin for CORS"`
	ToolRegistry               string `usage:"The tool reference for the tool registry" default:"github.com/orion101-ai/tools"`
	WorkspaceProviderType      string `usage:"The type of workspace provider to use for non-knowledge workspaces" default:"directory" env:"ORION101_WORKSPACE_PROVIDER_TYPE"`
	...
	AWSKMSKeyARN               string `usage:"The ARN of the AWS KMS key to use for encrypting credential storage" env:"ORION101_AWS_KMS_KEY_ARN" name:"aws-kms-key-arn"`
	EncryptionConfigFile       string `usage:"The path to the encryption configuration file" default:"./encryption.yaml"`
	KnowledgeSetIngestionLimit int    `usage:"The maximum number of files to ingest into a knowledge set" default:"1000" env:"ORION101_KNOWLEDGESET_INGESTION_LIMIT" name:"knowledge-set-ingestion-limit"`
...
const (
	...
	defaultToolsRegistry = "github.com/orion101-ai/tools"
)
...
func newGPTScript(ctx context.Context, workspaceTool, datasetsTool, toolsRegistry string) (*gptscript.GPTScript, error) {
	...
	if os.Getenv("WORKSPACE_PROVIDER_DATA_HOME") == "" {
		if err = os.Setenv("WORKSPACE_PROVIDER_DATA_HOME", filepath.Join(xdg.DataHome, "orion101", "workspace-provider")); err != nil {
			return nil, err
		}
	}
...
func New(ctx context.Context, config Config) (*Services, error) {
	...
	if os.Getenv("ORION101_OPENAI_MODEL_PROVIDER_API_KEY") == "" {
		_ = os.Setenv("ORION101_OPENAI_MODEL_PROVIDER_API_KEY", os.Getenv("OPENAI_API_KEY"))
	}
	...
	r, err := baaah.NewRouter("orion101-controller", &baaah.Options{
		DefaultRESTConfig: restConfig,
		Scheme:            scheme.Scheme,
		ElectionConfig:    leader.NewDefaultElectionConfig("", "orion101-controller", restConfig),
		HealthzPort:       -1,
	})
	if err != nil {
		return nil, err
