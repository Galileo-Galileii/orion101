...
const steps: OAuthFormStep<typeof schema.shape>[] = [
    ...
    },
    {
        type: "markdown",
        text:
            "### Step 5: Register OAuth App in Orion101\n" +
...
export const SlackOAuthApp = {
    ...
    disabledReason: disableConfiguration
        ? "Slack requires that redirect URLs start with `https`. Since this application is running on `http`, you will need to redeploy Orion101 using `https` in order to configure a custom Slack OAuth app."
        : undefined,
