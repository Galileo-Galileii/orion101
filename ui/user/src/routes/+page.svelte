<script lang="ts">
	...
	$effect(() => {
		let id = $assistants.find((assistant) => assistant.id === 'orion101')?.id;
		if (!id) {
...
>
	...
	>
		<DarkModeToggle />
		<a href="https://docs.orion101.ai" class="icon-button" rel="external">
			<Book />
		</a>
		<a href="https://github.com/orion101-ai/orion101" class="icon-button text-white hover:text-ablue-50">
			{#if $darkMode}
	...
	<div class="mx-auto flex flex-col items-center gap-8">
		<div class="flex items-end gap-4">
			{#if $darkMode}
				<img src="/images/orion101-logo-blue-white-text.svg" alt="orion101 icon" class="h-64 pb-4" />
			{:else}
				<img src="/images/orion101-logo-blue-black-text.svg" alt="orion101 icon" class="h-64 pb-4" />
			{/if}
		</div>
		...
		<div class="mt-32 flex items-center gap-4">
			...
			<pre><code
					...
					data-language="shell">docker run -p 80:8080 -e OPENAI_API_KEY orion101/orion101</code
				></pre>
		</div>
